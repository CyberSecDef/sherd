...
| Block-level incremental reparse (`FR-MD-004`) | ✅ | Every edit agrees node-for-node with a full parse: `2159 of 5400 edits reparsed incrementally`, the rest correctly refused. The defect found in the audit — a list reaching over a blank line to claim a line an edit had just indented — is fixed, and pinned by both a committed fuzz seed and a named case in `TestReparseTakesTheFastPathWhereItShould`. Reverting the fix fails both (`QA-012`). |
...
| ≥ 95% coverage on `pkg/format` (`QA-001`) | ✅ *(by 0.1 points)* | `make cover` → `pkg/format 95.1% (704/740 statements)`, `internal 87.9%`. Enforced by `scripts/check-coverage.sh` in `make check` and in CI, with its own self-test; before this it was a number taken by hand, and it had drifted 2.3 points from what PLAN.md recorded. **The margin is one statement.** Most of what is left uncovered is defensive — bounds checks on ranges that no input this corpus or 124 M fuzz executions can produce — so the next guard added to `pkg/format` may well be what turns the build red. The script now warns below half a point so that arrives as a notice rather than as a failure, and P0.2 brings enough new tested code to restore the margin. |
...
| 24 h fuzz, zero crashes | 🔄 **accruing — and it has already earned its keep** | The first nightly-length run, dispatched by hand at 120 min per target on 2026-08-22, **found a wrong incremental reparse 47 minutes in**: a link destination is the one inline construct that reaches across a blank line, so a ")" typed anywhere later closes a destination opened in an earlier block and folds every block between them into the link. The reparser had no guard for a construct reaching *backwards*, only for fences and definitions reaching forward. Fixed, with the input committed as a seed and a corpus case pinning what goldmark actually does with it. This is the argument for the criterion: 15 minutes found nothing, 47 found a defect that corrupts a file on the next save. Earlier evidence: 15 min per target on this tree — `FuzzReparse` 86.1 M executions at ~95 k/sec, `FuzzParse` 38.1 M — with zero crashes, growing the cached corpus by 85 interesting inputs. That is the figure today, and it is not 24 h. `.github/workflows/fuzz.yml` accrues it at two hours per target per night, so it arrives in about a week and then keeps going, which is the more useful property for a parser that is still being changed; `ci.yml` also fuzzes 60 s per target on every push, which is what catches a crasher on the day it is introduced. This row closes when the nightly total passes 24 h per target. |
...
case asserts — it had been scoring 667/667 against an HTML-only parser. And
